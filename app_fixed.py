"""
app.py  —  LexiSum AI Backend (FIXED)
=======================================
Pipeline:  Upload → Extract → Chunk → Summarise (DistilBART) → Translate (MarianMT) → Dashboard
...
# CRITICAL FIX: Increase timeout for large model downloads
...
def get_summarizer():
    """Load summarization model with proper error handling and progress indication"""
    global _summarizer
    if _summarizer is None:
        try:
            ...
            print("Size: ~1.2 GB | This may take 5-15 minutes")
            print("="*60 + "\n")
            
            # Download with explicit cache and timeout settings
            ...
            print(f"\n❌ ERROR loading summarization model: {e}")
            print("\nTROUBLESHOOTING:")
            print("1. Check your internet connection")
            print("2. Try running: pip install --upgrade transformers torch")
            print("3. Clear cache: rm -rf hf_models/")
...
def get_translator():
    """Load translation model with proper error handling"""
    global _translator
    if _translator is None:
        try:
            ...
            print(f"\n❌ ERROR loading translation model: {e}")
            raise
...
if __name__ == "__main__":
    print("\n" + "="*60)
    print("LEXISUM AI - STARTUP SEQUENCE")
    print("="*60)
    ...
    try:
        ...
        print(f"\n❌ STARTUP FAILED: {e}")
        print("\nThe server will not start until models are downloaded.")
